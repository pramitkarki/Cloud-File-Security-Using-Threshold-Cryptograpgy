...
from share import keygen, secgen
from upload import uploadImage, fileUrl
...
def main():
    print("---------------------------------------------------")
    print("- Cloud File Security With Threshold Cryptography -")
    print("---------------------------------------------------\n")
    L = 1024
    ...
    print()
    text = input("Enter File name to secure: ")
    text_id = input("Enter file content id: ")
    print()
    uploadImage(text, text_id)
    M = str.encode(text, "ascii")
    r, s = sign(M, p, q, g, x)
    print("\n -- Secret Recovery From Shared Keys --\n")
    N = 160 + secgen()
    if prevN == N and verify(M, r, s, p, q, g, y):
        print()
        print('----- Signature Verified -----\n')
        print('--Here is your unlocked file URL from cloud :', text)
        fileUrl(text, text_id)
    else:
        print("---- Rejected ----")
